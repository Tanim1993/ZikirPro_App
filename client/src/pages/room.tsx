...
export default function Room() {
  ...
  const { sendMessage } = useWebSocket({
    onMessage: (data) => {
      if (data.type === 'countUpdate' && data.data.roomId === roomId) {
        ...
        if (data.data.userId === user?.id && data.data.counter.todayCount % 100 === 0) {
  ...
  const incrementCountMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries(["/api/rooms", roomId, "leaderboard"]);
    },
    onError: (error: Error) => {
      if (isUnauthorizedError(error)) {
        toast({
  ...
  const userInLeaderboard = leaderboard.find((user: any) => user.userId === user?.id);
    ...
    <div className="min-h-screen bg-white">
      ...
      <div className="bg-gradient-to-r from-islamic-green to-islamic-green-light text-white p-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-center">
            <h2 className="text-xl font-bold font-amiri">{room.zikirName}</h2>
            <p className="text-green-100">{room.description || `${room.duration} Days Challenge`}</p>
          </div>
          <Button 
            variant="ghost"
            className="p-2 hover:bg-white hover:bg-opacity-20 rounded-lg text-white"
